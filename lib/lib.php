...
define('CRITERIA_ACTIVITY_COMPLETED', 250004); // added by G. Schwed, 2017-10
...
function block_dukreminder_build_navigation_tabs($courseid) {
    ...
    $rows[] = new tabobject('tab_new_reminder',
            new moodle_url('/blocks/dukreminder/sent_reminders.php',
                    array("courseid" => $courseid)),
            get_string('tab_sent_reminders', 'block_dukreminder'));
...
function block_dukreminder_get_pending_reminders() {
    ...
    // Hack: normally reminders should be deleted when a course is deleted.
...
function block_dukreminder_filter_users($entry) {
    ...
    // Get all students (= potential users).
    ...
    // Filter users by absolute date + course completion.
...
// ------------- work in progress -----------------------
    // Filter users by absolute date + activity completion
    // Send email if user has not completed this activity
/*
    if ($entry->dateabsolute > 0 && $entry->criteria == CRITERIA_ACTIVITY_COMPLETED) {
        mtrace("absolute + act_compl; courseid: $entry->courseid<br>"); // debug
        // Get activity
        //...
        ...
        foreach ($users as $user) {
            ...
            // Unset if user has completed
                unset($users[$user->id]);
            } else { $detail .= " => not completed!";
            }
            mtrace("... user $user->id: $detail<br>");
...
function block_dukreminder_get_mail_text($course, $users, $textteacher = null) {
    global $DB; // added 2018-02-26

    $userlisting = '';
    foreach ($users as $user) {
        $user = $DB->get_record('user', array('id' => $user->id)); // get full user object; added 2018-02-26
...
function block_dukreminder_get_criteria($criteriaid) {
    ...
    if ($criteriaid == CRITERIA_ACTIVITY_COMPLETED) {
        return get_string('activity_completed', 'block_dukreminder');
    }

    // Get item with pass grade
    if (($criteriaid > 1000000) AND ($criteriaid < 2000000)) { // 
    ...
    // Get activitiy with completion tracking
    if ($criteriaid > 2000000) {
        $activity_id = $criteriaid - 2000000;
        // Get all activities with completion tracking in this course
        ...
        return get_string('criteria_activity_completed', 'block_dukreminder') . ': ' .  $activity->name . " (" . $activity->modname . ")";
    }
 ...
 * @return 
 */
function dukreminder_supports($feature) {
    switch($feature) {
        case FEATURE_BACKUP_MOODLE2:          return false; // We want to implement backupfeatures. michaelpollak.
        default: return null;
    }
}
